...
// 🔧 Fixed for 1.12.3: real hook is 'bricks/element/render' (apply_filters( 'bricks/element/render', $render_element, $this )
// in includes/elements/base.php:2620). 'bricks/frontend/render_element' does not exist in the source.
add_filter( 'bricks/element/render', function( $html, $element_instance ) {
...
> ⚠️ **Không tồn tại trong Bricks 1.12.3 (site này)** — filter `bricks/frontend/render_loop` không có trong source (0 kết quả grep trong `includes/`). Đừng dùng.
...
> ⚠️ **Không tồn tại trong Bricks 1.12.3 (site này)** — `includes/elements/pagination.php` không có bất kỳ `apply_filters`/`do_action` nào (đã kiểm tra toàn bộ file). Ba filter dưới đây (`bricks/pagination/custom_logic`, `bricks/pagination/current_page`, `bricks/pagination/total_pages`) không có trong source. Giữ lại tham khảo nếu site nâng cấp sau này.
...
```php
// Modify query filter index arguments (confirmed: includes/query-filters-indexer.php:639)
add_filter( 'bricks/query_filters/index_args', function( $args, $source, $settings, $type ) {
    return $args;
}, 10, 4 );
```

> ⚠️ **Không tồn tại trong Bricks 1.12.3 (site này)** — filter `bricks/query_filters_indexer/post/{filter_source}` không có trong source. Hook thật để can thiệp trước khi index 1 post/user là action `bricks/query_filters/index_post/before` / `bricks/query_filters/index_user/before` (confirmed `do_action` tại `includes/query-filters.php:3215` và `:3637`) — đã liệt kê đúng ở mục "Query Filters Actions" bên dưới.
...
> ⚠️ **Không tồn tại trong Bricks 1.12.3 (site này)** — action `bricks/dynamic_data/tags_registered` không có trong source. Filter thật để loại trừ tag là `bricks/dynamic_data/exclude_tags` (confirmed `includes/integrations/dynamic-data/providers.php:173`).
...
> ⚠️ **Không tồn tại trong Bricks 1.12.3 (site này)** — action `bricks/render_popup_content/start` không có trong source (`includes/api.php` chỉ có `bricks/render_query_page/start` và `bricks/render_query_result/start`, cả hai đã liệt kê đúng ở trên).
