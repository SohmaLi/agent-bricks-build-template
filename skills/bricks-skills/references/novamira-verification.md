...
> Verified against Bricks 1.12.3 source (theme path: `.../wp-content/themes/bricks`) — 2026-07-14. Spot-checked (not a deep pass): meta keys, `BRICKS_VERSION`, and PHP class/method names referenced below all matched 1.12.3, with one exception flagged inline.
...
```php
// ⚠️ Không tồn tại trong Bricks 1.12.3 (site này) — tính năng của bản 2.x, xác nhận qua grep source (functions.php, includes/) không thấy 'bricks_element_manager' hay BRICKS_DB_ELEMENT_MANAGER ở đâu cả. Giữ lại tham khảo nếu site nâng cấp sau này.
