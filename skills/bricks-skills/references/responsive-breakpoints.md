...
> Verified against Bricks 1.12.3 source (theme path: .../themes/bricks) — 2026-07-14. This file checked out accurate; only the Settings-tab location for "Custom breakpoints" was corrected (see below). No other 2.x-only content found.

Source: `{template_dir}/includes/breakpoints.php`. Stored as option `bricks_breakpoints` (constant `BRICKS_DB_BREAKPOINTS`, `functions.php`). Custom breakpoints are gated behind `bricks_global_settings.customBreakpoints = true` (`Database::get_setting('customBreakpoints', false)`, `breakpoints.php` line 47/440) — all confirmed.
...
1. **Enable custom breakpoints:** Bricks > Settings > General tab > "Custom breakpoints" checkbox -> ON (confirmed in `includes/admin/admin-screen-settings.php`, inside `<table id="tab-general">`, not the Performance tab). This sets `customBreakpoints = true` in `bricks_global_settings`.
...
When `cssLoading == 'file'` (default), Bricks regenerates per-element CSS files into `{template_dir}/assets/css/elements/*.min.css` and the integrations in `{template_dir}/assets/css/integrations/*.min.css` whenever you save a page or update a breakpoint (confirmed in `includes/breakpoints.php::update_media_rule_width_in_css_files()`). Also regenerates `frontend.min.css` and `frontend-light.min.css`.
