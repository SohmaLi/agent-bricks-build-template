...
| `imageHeight` | number+unit → `img` | Chiều cao thumbnail |
| `imageWidth` | number+unit → `img` | Chiều rộng thumbnail |
...
| `overlayAlignItems` | align-items | Align dọc content trong overlay |
| `overlayJustifyContent` | justify-content | Align ngang content trong overlay |
| `contentWidth` | number+unit | Width content area |
| `contentPadding` | spacing | Padding content area |
| `contentBackground` | color | Background content area |

### Query Group (advanced)
| Key | Type | Mô tả |
|-----|------|-------|
| `queryMaxResultsInfo` | info | Thông báo giới hạn kết quả query |
