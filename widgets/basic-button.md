...
| Key | Giá trị ví dụ |
|-----|--------------|
| `_typography` | Màu text, font, size của label |
| `_background` | Background color / gradient |
| `_border` | Border + border-radius |
| `_padding` | `{"top":"12px","bottom":"12px","left":"32px","right":"32px"}` |
| `_width` | `"100%"` cho full-width button |
| `_cssCustom` | Hover effects, transitions |
...
```json
{
  ...
  "settings": {
    ...
    "_cssCustom": "%root% {\n  transition: all 0.2s ease;\n}\n%root%:hover {\n  background: #0056b3;\n  transform: translateY(-2px);\n  box-shadow: 0 8px 24px rgba(0, 124, 252, 0.3);\n}"
  }
}
```
