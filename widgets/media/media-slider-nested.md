...
| `perPage:mobile_portrait` | number      | perPage override trên mobile — **VERIFIED ✅** |
| `perPage:tablet_portrait` | number      | perPage override trên tablet                   |
| `perMove`                 | number      | Số slides di chuyển mỗi lần                    |
| `start`                   | number      | Index slide đầu (0-based)                      |
| `gap`                     | number+unit | Khoảng cách giữa slides                        |
| `gap:mobile_portrait`     | number+unit | Gap override trên mobile — **VERIFIED ✅**     |
...
> ⚠️ **CHECKBOX DEFAULT = ON:** `arrows` và `pagination` **mặc định BẬT** trong Bricks UI khi không set key.
>
> - Muốn TẮT → **KHÔNG set key này** trong settings khi dùng `update_content` full rebuild
> - `partial update` (action: `update`) KHÔNG clear key cũ trong DB — old value `true` vẫn còn
> - `update_content` full rebuild → key không có → default ON → vẫn bật! ❌
> - ✅ **Cách tắt duy nhất đúng:** Dùng CSS hide: `_cssCustom: "#brxe-[id] .splide__arrows { display:none!important; }"`

| Key                      | Type      | Mô tả                                      |
| ------------------------ | --------- | ------------------------------------------ |
| `arrows`                 | checkbox  | Hiện arrows — **default ON nếu không set** |
...
### ⚠️ Widget Hierarchy Trong Slide — BẮT BUỘC

Hierarchy rule: `section → container → block → [leaf widgets]`

| Trong slide dùng | ✅/❌ | Lý do |
|---|---|---|
| `block` để nhóm content | ✅ | Block = styled grouping wrapper |
| `heading`, `text-basic`, `text`, `image`, `button` trực tiếp | ✅ | Leaf widgets |
| `container` bên trong slide | ❌ | `container` = 1140px section-level wrapper, **KHÔNG phải** generic grouping |

**❌ SAI — dùng `container` bên trong slide:**
```
block "Slide 1"
  ├── image
  ├── container → text   ← SAI! container không thuộc về đây
  └── block (overlay)
```

**✅ ĐÚNG — dùng `block` để nhóm nếu cần:**
```
block "Slide 1" (card: border, padding, position:relative, overflow:hidden)
  ├── block       (bg overlay — position:absolute)
  ├── image
  ├── block       (text group — flex-col, gap)
  │   ├── text-basic  (label)
  │   └── text        (description)
  └── block       (glow overlay — position:absolute)
```

> ⛔ `container` CHỈ xuất hiện ở **depth 1** ngay dưới `section`. Bên trong slide (là một `block`) — chỉ dùng `block` và leaf widgets.
