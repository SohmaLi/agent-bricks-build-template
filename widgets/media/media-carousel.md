...
| `stopOnLastSlide` | checkbox | Dừng autoplay ở slide cuối |

### Image Group
| Key | Mô tả |
|-----|-------|
| `imageSize` | Kích thước ảnh WP (full, large, medium) |
| `imageDisable` | Tắt hiển thị ảnh (dùng khi type=posts) |
| `imageLightbox` | Link đến lightbox (media type only) |
| `imageLightboxSize` | Kích thước ảnh trong lightbox |
| `lightboxAnimationType` | Loại animation lightbox: `zoomIn`, `fade`... |
